...
import org.wint3794.ftc.pathfollower.drivebase.mecanum.CalculateVelocities
...
object VelocityChecker {
    ...
    fun checkMagnitude(vd: Double) {
        if (vd !in 0.0..1.0) throw InvalidChassisDirectiveException(
            "Check Magnitude."
        )
    }
    ...
    fun checkSpeed(vt: Double) {
        if (vt !in -1.0..1.0) throw InvalidChassisDirectiveException(
            "Check Speed."
        )
    }
    ...
    fun checkAngle(td: Double) {
        if (!(td <= 2 * PI && td >= 0)) throw InvalidChassisDirectiveException(
            "Check Angle."
        )
    }
    ...
    fun checkMecanumDirectives(directives: MecanumDirectives) {
        ...
        if (td > Math.PI) throw InvalidChassisDirectiveException(
            "Check Angle: $td"
        )
        if (vt !in -1.0..1.0) throw InvalidChassisDirectiveException(
            "Check Rotation: $vt"
    ...
    fun checkCoordinates(coordinate: JoystickCoordinates) {
        if (abs(coordinate.y) > 1) throw InvalidJoystickCoordinatesException(
            "Check Ordinate Coordinate: " + coordinate.y
        )
        if (abs(coordinate.x) > 1) throw InvalidJoystickCoordinatesException(
            "Check Abscissa Coordinate: " + coordinate.x
        )
