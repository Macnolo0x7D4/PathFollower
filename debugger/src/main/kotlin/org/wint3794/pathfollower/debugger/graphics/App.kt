package org.wint3794.pathfollower.debugger.graphics

import org.wint3794.pathfollower.debugger.util.CommandProcessor
import org.wint3794.pathfollower.debugger.geometry.Line
import org.wint3794.pathfollower.debugger.geometry.Point
import org.wint3794.pathfollower.debugger.util.Constants
...
import org.wint3794.pathfollower.debugger.Main
...
class App : Application() {
        ...
        object : AnimationTimer() {
            override fun handle(currentNanoTime: Long) {
                try {
                    ...
                    Screen.setDimensionsPixels(scene.width, scene.height)
                    canvas.width = Screen.fieldSizePixels
                    canvas.height = Screen.fieldSizePixels

                    fieldBackground.fitWidth = Screen.fieldSizePixels
                    fieldBackground.fitHeight = Screen.fieldSizePixels

                    debuggingLabel.maxWidth = scene.width * 0.5
                    debuggingLabel.text = "Robot Position:\nX: ${CommandProcessor.robotX.roundToInt()} , Y: ${CommandProcessor.robotY.roundToInt()}, Angle: ${Math.toDegrees(CommandProcessor.robotAngle).roundToInt()}°"
    ...
    private fun drawScreen(context: GraphicsContext) {
        context.clearRect(0.0, 0.0, Screen.widthScreen, Screen.heightScreen)
        drawRobot(context)
    ...
    private fun drawDebugPoints(context: GraphicsContext) {
        for (i in displayPoints.indices) {
            val displayLocation = Screen.convertToScreen(
                    Point(displayPoints[i].x, displayPoints[i].y))
            val radius = 5.0
        ...
        for (i in CommandProcessor.pointLog.indices) {
            val displayLocation = Screen.convertToScreen(
                    Point(CommandProcessor.pointLog[i].x,
    ...
    private fun drawDebugLines(gc: GraphicsContext) {
        for (i in displayLines.indices) {
            val displayLocation1 = Screen.convertToScreen(
                    Point(displayLines[i].x1, displayLines[i].y1))
            val displayLocation2 = Screen.convertToScreen(
                    Point(displayLines[i].x2, displayLines[i].y2))
            gc.lineWidth = 3.0
    ...
    private fun drawRobot(context: GraphicsContext) {
        ...
        Screen.setCenterPoint(Screen.centimetersPerPixel * Screen.widthScreen / 2.0,
                Screen.centimetersPerPixel * Screen.heightScreen / 2.0)

        val originInPixels = Screen.convertToScreen(Point(0.0, Constants.FIELD_SIZE))
        ...
        try {
            val bottomLeft = Screen.convertToScreen(Point(topLeftX, topLeftY))
