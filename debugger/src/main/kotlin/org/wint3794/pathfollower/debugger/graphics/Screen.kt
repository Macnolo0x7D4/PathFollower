...
object Screen {
    ...
    fun convertToScreen(p: Point): Point {
        val topLeft = topLeftScreenRealPosition
        val relativeFromTopLeft = Point(p.x - topLeft.x, topLeft.y - p.y)
        val percentX = relativeFromTopLeft.x / windowSizeInRealScale.x
        val percentY = relativeFromTopLeft.y / windowSizeInRealScale.y
        return Point(percentX * widthScreen, percentY * heightScreen)
    }

    private val topLeftScreenRealPosition: Point
        get() {
            val windowSizeReal = windowSizeInRealScale
            return Point(centerXReal - windowSizeReal.x / 2.0,
                    centerYReal + windowSizeReal.y / 2.0)
        }

    private val windowSizeInRealScale: Point
        get() = Point(widthScreen * centimetersPerPixel, heightScreen * centimetersPerPixel)
